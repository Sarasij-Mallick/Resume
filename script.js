...
// ── Card Shiver on Hover (one-shot per hover entry) ──────────────────────────
(function () {
  var CARD_SELECTORS = [
    '.about-card', '.path-card', '.tmpl-card', '.template-card',
    '.blog-card', '.feature-card', '.stat-card', '.testimonial-card',
    '.story-cta-card', '.tmpl-select-card'
  ].join(', ');

  function attachShiver() {
    document.querySelectorAll(CARD_SELECTORS).forEach(function (card) {
      if (card.dataset.shiverAttached) return;
      card.dataset.shiverAttached = '1';

      card.addEventListener('mouseenter', function () {
        card.classList.remove('card-shiver');
        void card.offsetWidth; // force reflow so animation restarts fresh
        card.classList.add('card-shiver');
      });

      card.addEventListener('animationend', function (e) {
        if (e.animationName === 'cardShiver') {
          card.classList.remove('card-shiver');
        }
      });
    });
  ...
  if (document.readyState === 'loading') {
    document.addEventListener('DOMContentLoaded', attachShiver);
  } else {
    attachShiver();
  }

  // re-attach for dynamically injected cards (e.g. carousel slides)
  var shiverObserver = new MutationObserver(attachShiver);
  shiverObserver.observe(document.body, { childList: true, subtree: true });
})();
